...
public class EventQueueDAO {
    ...
    public EventQueue createEventQueue(EventQueueRequest request) {
        return jdbc.queryForObject(
                "INSERT INTO event_queue (event_num, name, phone_num) VALUES (:eventNum, :name, :phoneNum) RETURNING *",
                new BeanPropertySqlParameterSource(request), rowMapper);
    }

    public EventQueue updateNextInLine(int servingNum) {
        return jdbc.queryForObject(
                "UPDATE event_queue SET next_in_line = CURRENT_TIME WHERE serving_num = :servingNum RETURNING *",
                new MapSqlParameterSource().addValue("servingNum", servingNum), rowMapper);
    }

    public EventQueue updateServed(int servingNum) {
        return jdbc.queryForObject(
                "UPDATE event_queue SET end_time = CURRENT_TIME WHERE serving_num = :servingNum RETURNING *",
                new MapSqlParameterSource().addValue("servingNum", servingNum), rowMapper);
    }
}